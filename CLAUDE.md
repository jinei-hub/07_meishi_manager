...
- **送信サイズは「Claude が見るサイズ」ちょうどに合わせる。** `services/imaging.py` の
  `resized_size()` は公式の reference implementation をそのまま実装したもの
  （辺の上限と視覚トークン上限 `ceil(w/28)*ceil(h/28)` の両方を満たす最大サイズ）。
  写真ではトークン上限が先に効くので、4:3 なら 2576px ではなく約 2212x1659 になる。
  **ここを合わせないと、返ってくる bbox の座標が縮小後基準になってズレる。**
- **複数枚を1枚の写真で撮っても、名刺ごとに切り出して保存する。** 読み取りと同時に
  各名刺の `bbox` を返させ（`ocr/extract.py`）、`crop_bbox()` で切り出す。
  API の呼び出しは増えない（同じリクエストに座標が乗るだけ）。
  座標は**絶対ピクセルで要求する**こと。公式に「正規化座標はうまく動かない」と
  明記されている（2026-09-13 に確認）。切り出しに失敗したら写真全体にフォールバックする。
- **裏面にも情報がある名刺は、表と裏を1回のリクエストで読む。** `extract_cards(...,
  back_image_bytes=...)` で「画像1: 表面 / 画像2: 裏面」とラベルを付けて2枚送り、
  1人分に統合させる（両面に同じ項目があれば日本語表記を優先）。裏面の位置は `back_bbox`。
  API 呼び出しは増えないが、画像1枚分の入力トークンが増える（裏面を撮ったときだけ）。
  裏面は**表・裏とも名刺1枚ずつ**の前提。複数枚だと組み合わせを取り違えるので画面で警告する。
- **保存画像は表の下に裏を縦に並べた1枚にする**（`stack_vertical()`）。DB の画像列が
  1つしかなく、Alembic 未導入で列を足せないため。スマホは縦スクロールなので縦並び。
